...
def build_handler(usage_va):
    ...
    a.b("ploop")
    a.label("pdone")
    e(W(0xD29FFFEB))  # movz x11,#0xFFFF (mask lo32)
    e(W(0xF2BFFFEB))  # movk x11,#0xFFFF,lsl#16
    e(W(0x8A0B014B))  # and x11,x10,x11
    e(W(0xEB0B015F))  # cmp x10,x11
    a.bcond(1, "fulladdr")  # b.ne fulladdr (top bits set: full VA)
    e(W(0xF2FFFFEA))  # movk x10,#0xFFFF,lsl#48 (LK high bits)
    a.label("fulladdr")
    e(mov64(11, LK_LO))
    e(W(0xEB0B015F))  # cmp x10,x11
    a.bcond(3, "usage")  # b.lo usage
    e(mov64(11, LK_HI))
    e(W(0xEB0B015F))  # cmp x10,x11
    a.bcond(2, "usage")  # b.hs usage
    e(W(0xAA0A03F4))  # mov x20,x10 (read ptr)
    e(movz(22, 16, 0))  # mov w22,#16 (lines)
    a.label("line")
    e(W(0xD10103A9))  # sub x9,x29,#0x40 (buf)
    e(W(0xD2800008))  # mov x8,#0 (col)
    a.label("col")
    e(W(0x5280082B))  # mov w11,#0x41 BISECT (expect 41 x16)
    e(W(0x531C7D6C))  # lsr w12,w11,#4 (hi nibble)
    e(W(0x1100C18C))  # add w12,w12,#0x30
    e(W(0x7100259F))  # cmp w12,#9
    a.bcond(9, "hi_ok")  # b.ls hi_ok
    ...
    e(W(0x510006D6))  # sub w22,w22,#1
    e(W(0x710002DF))  # cmp w22,#0
...
EXPECTED_MNEMONICS = (
    ...
    "mov movk and cmp b.ne movk mov movk movk movk cmp b.lo "
    "mov movk movk movk cmp b.hs "
    "mov mov sub mov mov lsr add cmp b.ls add strb and add cmp b.ls "
    "add strb add cmp b.ne strb sub adrp add bl add sub cmp b.ne mov b "
    "adrp add adrp add bl mov add ldp ldp ldp ret"
).split()

NAME_STR = b"readdump\x00"
USAGE_STR = b"usage: fastboot oem readdump <hexaddr>\x00"
...
NEED_OPS = {
    ...
    "mov w11,#0x41": "bisection constant",
...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        a1 = build_handler(LK_LO)  # dummy usage_va, same length
        c1 = a1.resolve()
        name_off = align8(CAVE + len(c1))
        usage_off = name_off + 16
        end = usage_off + 48
        assert end - CAVE <= 0xF84, "does not fit cave"
        code = build_handler(BASE + usage_off).resolve()
