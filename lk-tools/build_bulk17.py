#!/usr/bin/env python3
"""readdump bulk-17: `readdump <hexaddr>` INFO-hex + `readdump <hexaddr>+`
16KB DATA dump. Same regex slot, same gates, VALID re-sign.

Bulk v2 (stub-based): header via control stub 0x7E00, ONE 16KB send via
bulk stub 0x7E24, flush via 0x7E34 -- the eMMC sender's proven path.
Supersedes bulk16's raw-vtable (+0x30 request-struct) convention, which
crashed every send. No loops except the proven col loops.
"""
import struct
import sys
...
def build_handler(usage_va, tbl_va, deny_va, data_va, empty_va, lenhex_va):
    ...
    e(W(0x7100017F))
    a.bcond(1, "usage")
    e(W(0x5280002D))  # mov w13,#1 (bulk flag; VERIFIED word)
    a.label("pdone")
    e(movz(11, 0x5000, 1))
    e(cmp_reg64(10, 11))
    a.bcond(3, "tbl_raw")
    ...
    e(cmp_reg64(10, 13))
    a.bcond(3, "tnext")
    # end select: bulk? +0x3FFF : +255
    e(W(0x710001BF))  # cmp w13,#0 (VERIFIED word)
    a.bcond(0, "is_info")
    e(movz(15, 0x3FFF, 0))
    ...
    # TRANSPORT v2 (stub-based): the USB vtable method +0x30 takes a
    # REQUEST STRUCT (ramdump.md S18), NOT (buf,len) -- every bulk16 send
    # crashed on that wrong assumption. The eMMC sender (file 0xA840+)
    # proves the correct bulk path: stub 0x7E00 (+0x30, buf+len control),
    # stub 0x7E24 (+0x40, buf+len raw DATA), stub 0x7E34 (flush/status).
    # All three stubs live inside lk.bin (verifiable); no struct, no
    # F8DEF4 dependency. Single 16KB send from mapped-sure LK-VA first.
    a.label("bulk_go")
    ...
    # Proven pattern: eMMC pre-loop packet (mov x1,x0; mov x0,sp; bl 7E00).
