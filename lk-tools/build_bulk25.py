...
def build_handler(usage_va, tbl_va, deny_va, data_va, empty_va, lenhex_va,
    ...
    # header via RESPONDER with FULL header as tag: responder(tag,msg)
    # emits tag-packet then msg-packet (S17 split). tag="DATA00004000"
    # (static HDR12) + msg="" -> line 1 IS a valid DATA header -> host
    # enters DATA phase and consumes the 7E24 bytes. (F29978 A+B framed
    # nothing: EOVERFLOW but alive. Responder calls are proven-safe, so
    # worst case here is another poison+reboot, never a crash.)
    a.adrp_add(0, hdr12_va)  # x0 = "DATA00004000" tag (static, contiguous)
    a.adrp_add(1, empty_va)  # x1 = "" msg
    a.bl_abs(RESPONDER)
