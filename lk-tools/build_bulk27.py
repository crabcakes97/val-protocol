#!/usr/bin/env python3
"""readdump bulk-21: b19 + zero-stack-writes bulk_go (rung A proved the
stack-built header faults though benign: addr in x19, header = static
cave string HDR12). INFO-hex + 16KB DATA via raw 7E24/7E34 stubs.
"""
import struct
import sys
...
def build_handler(usage_va, tbl_va, deny_va, data_va, empty_va, lenhex_va,
    ...
    # BARRIER: 7E34 between header and data. If it blocks till the pipe
    # is idle, the host parses the DATA line ALONE (like eMMC's slow
    # disk-read gap); back-to-back sends may glue header+data into one
    # chunk that the client's parser rejects. Cheap, proven-safe call.
