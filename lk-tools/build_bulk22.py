...
def build_handler(usage_va, tbl_va, deny_va, data_va, empty_va, lenhex_va,
    ...
    # header send: x0 = static "DATA00004000" (12B, one packet).
    # header via stock formatted sender F29978(-1, "DATA%08x", len):
    # eMMC's site-3 pattern. Emits the DATA line on the RESPONSE channel
    # (single packet) -> host enters DATA phase -> 7E24 bytes get consumed.
    # (b21 header-via-7E24 moved bytes but never framed: EOVERFLOW proof.)
    e(W(0x12800000))  # mov w0,#-1 (stock word, cf A804/A97C)
    a.adrp_add(1, BASE + 0xDDD40)  # x1 = "DATA%08x" (in-file fmt)
    e(W(0x52880002))  # mov w2,#0x4000 (len; render-verified)
    a.bl_abs(BASE + 0xF29978)
