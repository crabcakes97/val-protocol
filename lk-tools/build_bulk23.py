...
def build_handler(usage_va, tbl_va, deny_va, data_va, empty_va, lenhex_va,
    ...
    e(W(0x52800020))
    a.b("epilogue")
    # bulk: addr in x19 -> static header -> ONE 16KB send -> OKAY/FAIL
    # TRANSPORT v4 (zero stack writes): rung A proved the stack-built
    # header faults though every word is benign. So: addr rides in x19
    # (callee-saved, argv dead after parse), header is a STATIC cave
    # string. No stur/ldur/ldr in bulk_go at all.
    a.label("bulk_go")
    e(mov_reg64(19, 10))  # mov x19,x10 (addr; gate-checked)
    # header via stock formatted sender at file 0x29978 (VA ...50F29978):
    # F29978(w0=ch, x1=tag, x2=fmt, x3+=varargs) per eMMC sites (site-3 =
    # DATA line). PHANTOM-F LESSON: session shorthand "0xF29978" is the VA
    # tail; offset is 0x29978. b22 bl'd BASE+0xF29978 (unmapped!) -> crash.
    # (b21 header-via-7E24 moved bytes but never framed: EOVERFLOW proof.)
    e(W(0x12800000))  # mov w0,#-1 (stock word, cf A804/A97C)
    a.adrp_add(1, empty_va)  # x1 = tag "" (static cave empty)
    a.adrp_add(2, BASE + 0xDDD40)  # x2 = "DATA%08x" (in-file fmt)
    e(W(0xD2880003))  # mov x3,#0x4000 (len vararg; render-verified)
    a.bl_abs(BASE + 0x29978)
...
NEED_OPS = {
    ...
    "mov w0,#-1": "fmt channel",
    "mov x3,#0x4000": "fmt len vararg",
    "mov x0,x19": "bulk buf",
...
def main():
    ...
    z = LK_LO
    a1 = build_handler(z, z, z, z, z, z, z)
    c1 = a1.resolve()
    ...
    code = build_handler(BASE + usage_off, BASE + tbl_off, BASE + deny_off,
                          ...
                          BASE + lenhex_off, BASE + hdr12_off).resolve()
