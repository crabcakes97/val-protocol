#!/usr/bin/env python3
"""readdump bulk-29: bulk28 + USB-page window for factory-flag hunt.
Same INFO-hex + 16KB DATA vehicle; extra 16KB window
[0xFFFF000051052000,0xFFFF000051056000) covering the proven USB vtable
page ([0x51052280] deref'd by stock on every command) + factory flag
byte [0x51052668] written by the 0x17730 Entering-factory path.
MMU reason: same 4K page as the working USB send pointer, so mapped.
Single-row probe discipline still applies; faults are reboot-grade.
"""
import struct
import sys
...
from build_readdump_v1 import (
    ...
    BASE, CAVE, RESPONDER, INFO_TAG_OFF, REGEX_ROW, align8, WINDOWS as _BASE_WINDOWS,
    LK_LO,
)

# bulk29: base windows + USB-page window (MMU reason above).
WINDOWS = _BASE_WINDOWS + (
    (0xFFFF000051052000, 0xFFFF000051056000),
)
...
HDR12 = b"DATA00004000\x00"  # static contiguous DATA header (b21: no stack build)
HDR64 = b"DATA00004000\x00"  # same bytes, 64B-aligned cave offset (b26: DMA-align bet)
EMPTY_STR = b"\x00"
...
def build_handler(usage_va, tbl_va, deny_va, data_va, empty_va, lenhex_va,
    ...
    e(W(0x52800020))
    a.b("epilogue")
    # bulk: addr in x19 -> eMMC-exact recipe -> ONE 16KB send -> OKAY/FAIL
    # TRANSPORT v5 (eMMC recipe): snprintf+F8E7D4+7E00 DECODED (8C89C/8E7D4
    # bodies in-file; F8DEF4=memcpy so S18's "request struct" is DEAD).
    # eMMC: text-header via 7E00 (+0x30, timeout -1 inside) + data via
    # 7E24 + flush 7E34. ALIGNMENT BET: stock aligns frames to 64B
    # (responder `and sp,...c0`); our 16B frame may DMA-fault. So the
    # header here is STATIC at a 64B-ALIGNED cave offset (HDR64), data
    # addr (LK base) is already 64B-aligned. Zero stack writes (x19 addr).
    a.label("bulk_go")
    e(mov_reg64(19, 10))  # mov x19,x10 (addr; gate-checked)
    # header send via control stub 7E00 (x0=buf, w1=len; w2=-1 inside) --
    # byte-identical role to eMMC's pre-loop packet.
    a.adrp_add(0, hdr64_va)  # x0 = 64B-aligned "DATA00004000"
    e(W(0x52800181))  # mov w1,#0xc (render-verified)
    a.bl_abs(BASE + 0x7E00)
    e(W(0x7100001F))  # cmp w0,#0 (<0 == error bit31)
    a.bcond(0xB, "send_fail")  # b.lt fail
    # NO BARRIER: wire tap proved 7E34-with-no-prior-send FAILS (b27:
    # header then instant FAIL, zero data bytes). eMMC skips 7E34 before
    # the first send (w28==0) for exactly this reason. Straight to data.
    # data send via bulk stub 7E24 (x0=addr in x19, w1=len).
    e(W(0xAA1303E0))  # mov x0,x19 (gate-checked)
...
NEED_OPS = {
    ...
    "mov x19,x10": "bulk addr to calleesaved",
...
def main():
    ...
    z = LK_LO
    a1 = build_handler(z, z, z, z, z, z, z, z)
    c1 = a1.resolve()
    ...
    hdr12_off = align8(lenhex_off + 16)
    tbl_off = align8(hdr12_off + 16)
    tbl = b"".join(struct.pack("<QQ", lo, hi) for lo, hi in WINDOWS)
    hdr64_off = (tbl_off + len(tbl) + 0x3F) & ~0x3F  # 64B-aligned (DMA bet)
    end = hdr64_off + 16
    ...
    code = build_handler(BASE + usage_off, BASE + tbl_off, BASE + deny_off,
                          ...
                          BASE + lenhex_off, BASE + hdr12_off,
                          BASE + hdr64_off).resolve()
    ...
    payload[hdr12_off:hdr12_off + 16] = HDR12.ljust(16, b"\x00")
    payload[tbl_off:tbl_off + len(tbl)] = tbl
    payload[hdr64_off:hdr64_off + 16] = HDR64.ljust(16, b"\x00")
    assert (BASE + hdr64_off) & 0x3F == 0, "HDR64 not 64B-aligned"
