...
WINDOWS = (
    ...
    (0x500000, 0x520000),
    # USB-globals + region registry + per-CPU (~0xFFFF0000510xxxxx).
    # tpidr struct sits at 0x512FFF78 -- row MUST cover through 0x5130.
    (0xFFFF000051000000, 0xFFFF000051400000),
    ...
    # LINEAR-e00 SPOT-READS: registry e00 = phys [0x50F00000,+288MB] as
    # Normal (PROVEN mapped at 0x52000000+). 16MB starter row; widen by
    # doubling after each proven extent. Single-probe discipline per page.
    (0xFFFF000052000000, 0xFFFF000053000000),
)
...
def build_handler(usage_va, tbl_va, deny_va, m1_va, m2_va, m3_va):
    a = Asm(BASE + CAVE)
    e = a.emit
    ...
    # MARKER M1 (append done; if NO further output -> fault in append!).
    a.adrp_add(0, BASE + INFO_TAG_OFF)
    a.adrp_add(1, m1_va)
    a.bl_abs(RESPONDER)
    # 2) map(VA) via stock walker with FABRICATED sub-struct (tpidr chain
    # is null at runtime). Layout from map-body loads: root@+0x08 (TTBR1,
    # live via FA), flag@+0x10 (0), base@+0x18 (0) + size@+0x20 (~0 =
    # cover-all; range-check passes, -37 only if not). X = x29-0x38.
    e(W(0xAA1F03EB))  # mov x11,xzr
    e(stur64(11, 29, -0x38))  # [X+0x00]=0 (gate-checked)
    e(movz(11, 0x300, 0))  # mov x11,#0x300 (helper-computed)
    e(movk(11, 0x51, 1))  # movk x11,#0x51,lsl#16 (helper-computed)
    e(stur64(11, 29, -0x30))  # [X+0x08]=TTBR1 root (gate-checked)
    e(stur64(31, 29, -0x28))  # [X+0x10]=flag 0 (gate-checked)
    e(stur64(31, 29, -0x20))  # [X+0x18]=base 0 (gate-checked)
    e(W(0x9280000B))  # mov x11,#-1 (all-ones; gate-checked)
    e(stur64(11, 29, -0x18))  # [X+0x20]=size ~0 (gate-checked)
    e(W(0xD100E3A0))  # sub x0,x29,#0x38 (X; gate-checked)
    ...
    e(W(0xD10103A2))  # sub x2,x29,#0x40 (out-scratch, INFO-buf area, free on FD)
    # x3 = DESCRIPTOR OUT-pointer (NOT flags!): map stores the new leaf
    # desc via [x19=x3] (0x69D4/0x6A0C str). x3=0 (remap1-4) = NULL DEREF
    # = the fab-struct crash. Point it at free frame [x29-0x10] instead.
    e(W(0xD10043A3))  # sub x3,x29,#0x10 (desc-scratch; gate-checked)
    a.bl_abs(BASE + 0x6844)
    e(W(0x7100001F))  # cmp w0,#0
    a.bcond(1, "mapfail")  # b.ne mapfail (map refused: print code)
    # MARKER M2 (map RETURNED 0; fault after this = TLBI/read, not map).
    a.adrp_add(0, BASE + INFO_TAG_OFF)
    a.adrp_add(1, m2_va)
    a.bl_abs(RESPONDER)
    ...
    # MARKER M3 (TLBI done; fault after this = the read-back = unmapped).
    a.adrp_add(0, BASE + INFO_TAG_OFF)
    a.adrp_add(1, m3_va)
    a.bl_abs(RESPONDER)
    ...
    a.b("sr_fmt")
    a.label("sr_tpidr")
    # FE: raw tpidr_el1 (which CPU struct? null? garbage?). mrs is safe.
    e(W(0xD538D08B))  # mrs x11,tpidr_el1 (stock family; gate-checked)
    a.b("sr_fmt")
    a.label("sr_tp38")
    # FF: [tpidr+0x38] (the map-fn struct ptr; null at runtime?). Guarded
    # for zero; nonzero-garbage faults reboot-grade (run FE first!).
    e(W(0xD538D088))  # mrs x8,tpidr_el1 (gate-checked)
    a.cbz(8, "sr_tpnull")
    e(W(0xF9401D0B))  # ldr x11,[x8,#0x38] (gate-checked)
    a.b("sr_fmt")
    a.label("sr_tpnull")
    e(W(0xAA1F03EB))  # mov x11,xzr (null marker; gate-checked)
    a.b("sr_fmt")
    a.label("sr_fmt")
...
NEED_OPS = {
    ...
    "cmp x10,#0xfe": "sentinel tpidr",
    "cmp x10,#0xff": "sentinel tp38",
    "mrs x11,tpidr_el1": "tpidr raw",
    "mrs x8,tpidr_el1": "tpidr chain",
    "ldr x11,[x8,#0x38]": "struct ptr deref",
    "mov x11,xzr": "null marker",
    ...
    "mrs x8,tpidr_el1": "tpidr chain (FE/FF only)",
    "mov x11,#0x300": "fab root lo",
    "movk x11,#0x51,lsl#16": "fab root hi",
    "stur x11,[x29,#-0x30]": "fab +0x08 root",
    "stur x11,[x29,#-0x38]": "fab +0x00 zero",
    "stur xzr,[x29,#-0x28]": "fab +0x10 flag",
    "stur xzr,[x29,#-0x20]": "fab +0x18 base",
    "mov x11,#-1": "fab size allones",
    "stur x11,[x29,#-0x18]": "fab +0x20 size",
    "sub x0,x29,#0x38": "fab struct ptr",
    "mov x1,xzr": "map VA lo",
    "movk x1,#0xc000,lsl#16": "map VA mid",
    "movk x1,#0xffff,lsl#48": "map VA hi",
    "sub x2,x29,#0x40": "map out-scratch",
    "sub x3,x29,#0x10": "map desc-scratch",
...
USAGE_STR = b"usage: oem readdump <a>|f8-ff [v9]\x00"
...
def main():
    ...
    payload[m1_off:m1_off + 8] = b"M1\x00".ljust(8, b"\x00")
    payload[m2_off:m2_off + 8] = b"M2\x00".ljust(8, b"\x00")
    payload[m3_off:m3_off + 8] = b"M3\x00".ljust(8, b"\x00")
