...
def build_handler(usage_va, tbl_va, deny_va, data_va, empty_va):
    ...
    e(W(0xD10143FF))  # sub sp,sp,#0x50 (proven unrolled shape)
    ...
    a.label("bulk_go")
    e(W(0xF81C03AA))  # str x10,[x29,#-0x40] (SNAPSHOT addr: regs die at bl)
    e(W(0xF81C83B0))  # str x16,[x29,#-0x38] (SNAPSHOT len)
    e(W(0xF81B83B0))  # str x16,[x29,#-0x50] (len bytes for header)
    ...
    a.bl_abs(RESPONDER)
    # ---- raw chunk loop ----
    e(W(0xF85C03B4))  # ldr x20,[x29,#-0x40] (reload addr)
    e(W(0xF85C83B6))  # ldr x22,[x29,#-0x38] (reload len)
    ...
    a.b("epilogue")
    # ---- info_path: BYPASS TEST (blocks skipped) ----
    a.label("info_path")
    a.b("usage")  # TEMP: prove entry/dispatch without running blocks
    a.label("info_real")
    e(mov_reg64(20, 10))  # mov x20,x10 (read ptr, pre-bl: x10 live here)
    for _ln in range(16):
        a.label(f"iline{_ln}")
    ...
    e(W(0x2A1F03E0))
    a.label("epilogue")
    e(W(0x910143FF))  # add sp,sp,#0x50 (matches shrunk frame)
...
NEED_OPS = {
    ...
    "stur x16,[x29,#-0x38]": "len snapshot to stack",
    "ldur x20,[x29,#-0x40]": "addr reload post-call",
    "ldur x22,[x29,#-0x38]": "len reload post-call",
