...
def build_handler(usage_va, tbl_va, deny_va):
    ...
    a.b("ploop")
    a.label("pdone")
    # sysreg probe: addrs ..F8..FB return CPU registers (read-only, safe).
    # Line 1's first 16 hex chars = u64 LE value. EL2 regs NEVER touched
    # (trap risk); check CurrentEL (..F8) first if needed later.
    e(mov64(11, 0xFFFFFFFFFFFFFFF8))
    e(cmp_reg64(10, 11))
    a.bcond(3, "not_sysreg")  # b.lo not_sysreg
    e(W(0x1200094B))  # and w11,w10,#7 (gate-verify; index 0..3, else usage)
    e(W(0x7100017F))  # cmp w11,#0 (VERIFIED bytes 7f010071)
    a.bcond(0, "sr_cur")  # b.eq CurrentEL
    e(W(0x7100057F))  # cmp w11,#1 (VERIFIED)
    a.bcond(0, "sr_t0")  # b.eq TTBR0_EL1
    e(W(0x7100097F))  # cmp w11,#2 (VERIFIED)
    a.bcond(0, "sr_t1")  # b.eq TTBR1_EL1
    e(W(0x71000D7F))  # cmp w11,#3 (VERIFIED)
    a.bcond(1, "usage")  # b.ne usage (..FC..FF undefined)
    e(W(0xD538A200))  # mrs x0, MAIR_EL1 (gate-verify)
    a.b("sr_store")
    a.label("sr_t1")
    e(W(0xD5382020))  # mrs x0, TTBR1_EL1 (S3_0_C2_C0_1, VERIFIED render)
    a.b("sr_store")
    a.label("sr_t0")
    e(W(0xD5382000))  # mrs x0, TTBR0_EL1 (gate-verify)
    a.b("sr_store")
    a.label("sr_cur")
    e(W(0xD5384240))  # mrs x0, CurrentEL (gate-verify)
    a.label("sr_store")
    e(W(0xF81B03A0))  # stur x0,[x29,#-0x50] (python-built, gate-rendered)
    e(W(0xD10143B4))  # sub x20,x29,#0x50 (slot addr; VERIFIED shape)
    a.b("slive0")
    a.label("not_sysreg")
    ...
    for _ln in range(16):
        ...
        if _ln == 0:
            ...
            a.label("slive0")  # sysreg path lands here (x20 preset)
...
EXPECTED_MNEMONICS = (
    ...
    "mov movk movk movk cmp b.lo and cmp b.eq cmp b.eq cmp b.eq cmp b.ne mrs b mrs b mrs b mrs stur sub b "
...
NEED_OPS = {
    ...
    "and w11,w10,#7": "sysreg index mask",
    "cmp w11,#0": "sysreg idx0 check",
    "cmp w11,#1": "sysreg idx1 check",
    "cmp w11,#2": "sysreg idx2 check",
    "cmp w11,#3": "sysreg idx3 check",
    "mrs x0, currentel": "EL read",
    "mrs x0, ttbr0_el1": "table base read",
    "mrs x0, ttbr1_el1": "table1 base read",
    "mrs x0, mair_el1": "attr table read",
    "stur x0,[x29,#-0x50]": "reg spill BELOW line buf",
    "sub x20,x29,#0x50": "slot addr for dump",
}
