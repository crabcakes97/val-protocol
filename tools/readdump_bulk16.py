#!/usr/bin/env python3
"""readdump bulk-16k: `readdump <hexaddr>` INFO-hex + `readdump <hexaddr>+`
16KB DATA dump. Same regex slot, same gates, VALID re-sign.

Bulk: snapshots (stack) -> header const via responder -> ONE 16KB vtable
send -> OKAY/FAIL. No loops except the proven col loops.
"""
import struct
import sys
...
def build_handler(usage_va, tbl_va, deny_va, data_va, empty_va, lenhex_va):
    ...
    # packet!) then pushed via the RAW vtable send. The responder may
    # split tag+text into two packets, which breaks the client's DATA
    # size parse (observed: OKAY but 0 bytes + poisoned channel).
    ...
    e(W(0xF9414328))  # ldr x8,[x25,#0x280]
    e(W(0xF9418108))  # ldr x8,[x8,#0x30]
    e(W(0xD10143A0))  # sub x0,x29,#0x50 (header buf; gate-verify)
    e(W(0x52800181))  # mov w1,#12 (gate-verify: 0xC<<5|1)
    e(W(0xD2827102))  # mov w2,#0x1388
    e(W(0xD63F0100))  # blr x8 (push 12B header packet)
    ...
    e(W(0xD2800000 | (0x4000 << 5) | 9))  # movz w9,#0x4000 (gate-verify)
...
NEED_OPS = {
    ...
    "ldr x8,[x25,#0x280]": "gadget struct",
    "ldr x8,[x8,#0x30]": "bulk method",
    ...
    "blr x8": "bulk call",
    "mov x2,#0x1388": "timeout",
    "mov x1,#0x4000": "chunk len",
    "mov x9,#0x4000": "expect len",
    "cmp w0,w9": "sent check",
